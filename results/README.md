...
- **[`PRODUCTION-RUNS.md`](PRODUCTION-RUNS.md)** — **the register**: every job whose records are
  here, with its cells, seeds, frame span, evaluator revision, policy modes, headline endpoint
  numbers and file links, plus the caveats that say what it cannot see. Generated by
  `scripts/production_run_register.py`; run it with `--check` to prove it is current. **Read this
  before reading the raw files** — 70 opaque `<job>__records.jsonl` names are not an inventory.
