...
- **status** STOPPED by watch budget 18:37 MSK during endpoint pass 2. Collected: 569 rows installed at results/records/card0-20260909-035152__records.jsonl (484 curve + 44 endpoint/native + 41 of 44 endpoint/mode). Evaluator ledger NOT written -- paired=False, 3 eval-hard mode rows missing. — **status recorded 243h ago**
...
- **eval** curve 3 ep/scene, endpoint 20 ep/scene, policy modes `native,mode`
- **status** complete: training 600k, then curve (528 rows) and endpoint (88 rows, both policy modes) re-evaluated offline and collected as reeval-v214-ppg-curve and reeval-v214-ppg-endpoint — **status recorded 59h ago**
...
- **eval** curve 3 ep/scene, endpoint 20 ep/scene, policy modes `native,mode`
- **status** yielded at 41% (251,904 frames): co-tenant took ~6.2 GiB in 20s, free fell 6422 -> 224 MiB, below the 4000 floor. Our own peak was 2,640 MiB against a 5,000 cap and never moved. 5 checkpoints retained (51200-251904), each separately evaluable. — **status recorded 76h ago**
...
- **eval** curve 3 ep/scene, endpoint 20 ep/scene, policy modes `native,mode`
- **status** failed — **status recorded 76h ago**
...
- **eval** curve 3 ep/scene, endpoint 20 ep/scene, policy modes `native,mode`
- **status** complete: CELL EXIT=0, NATIVE_CELL_COMPLETED 11:16 on 2026-09-17; 910 rows delivered and collected to results/records/card1-20260916-203537__records.jsonl (curve 528, endpoint 44 native + 44 mode, plus training-curve rows) — **status recorded 59h ago**
...
- **eval** curve 3 ep/scene, endpoint 20 ep/scene, policy modes `native,mode`
- **status** complete: NATIVE_CELL_COMPLETED 18:43 on 2026-09-17; curve 484/484 and endpoint 88/88 (44 native + 44 mode); 598 rows collected to results/records/card1-20260916-213222__records.jsonl — **status recorded 49h ago**
...
- **eval** curve 3 ep/scene, endpoint 20 ep/scene, policy modes `native,mode`
- **status** failed — **status recorded 62h ago**
...
- **eval** curve 3 ep/scene, endpoint 20 ep/scene, policy modes `native,mode`
- **status** failed — **status recorded 54h ago**
...
- **status** failed: host crashed ~21:25 MSK (unclean, rebooted 22:05; all tenants' containers lost). Cell had reached frame 9000 in 35 min at ~2.4 FPS with GPU util 0 -- far below the ~14 FPS its 12 h training budget needs. No checkpoint (first save at 50k). — status recorded under an hour ago
...
**10 run(s) above have produced no record file here.** Until they are collected their numbers exist only on the host, and nothing in this repository can be used to check them.
