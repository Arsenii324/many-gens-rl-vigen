...
# [Claude 2026-09-20] Real cells have now trained on the V100 host. `measured-v100-throughput.json`
# is the one place those measurements are recorded (each entry names its run id, what was measured
# and under what conditions -- see the file's own `_comment`); this module only reads its derived
# `per_baseline` block, never the ambient host. Absent entirely (e.g. a fresh checkout before any
# V100 cell has run), every baseline stays `UNMEASURED_ON_V100`, exactly as before this file existed.
_MEASURED_V100_PATH = _HERE.joinpath("measured-v100-throughput.json")
MEASURED_V100_PER_BASELINE: dict = (
    json.loads(_MEASURED_V100_PATH.read_text()).get("per_baseline", {})
    if _MEASURED_V100_PATH.is_file() else {}
)
...
def v100_schedule(frames: int, seeds: list[int]) -> dict:
    ...
    for baseline in order:
        ...
        # [Claude 2026-09-20] `measured-v100-throughput.json`'s `per_baseline` block is the one
        # place a real measurement is allowed to override `UNMEASURED_ON_V100` -- and only when it
        # names a non-null rate. A baseline whose only sample is flagged defective (`svea`, see
        # that file's own entry) is deliberately EXCLUDED there, so `.get("frames_per_second")`
        # returns None for it and this falls through to the unmeasured default unchanged.
        _v100_measurement = MEASURED_V100_PER_BASELINE.get(baseline) or {}
        _v100_fps = _v100_measurement.get("frames_per_second")
        if _v100_fps is not None:
            _v100_throughput_source = _v100_measurement.get("source") or "measured"
        else:
            _v100_throughput_source = "UNMEASURED_ON_V100"
        rows.append({
            "baseline": baseline,
            "family": family,
            ...
            "v100_completed_frames_per_second": _v100_fps,
            "throughput_source": _v100_throughput_source,
            "t4_completed_frames_per_second_reference": MEASURED_FPS_GT4_1.get(baseline),
            "t4_reference_basis": FPS_BASIS.get(baseline),
        })
    # [Claude 2026-09-20] A flat BLOCKED string read as "zero measured" even after four cells had
    # completed on this host. Machine-checkable, not prose: `PARTIAL_V100_THROUGHPUT n/total` names
    # exactly how many rows above carry a real rate, so a reader (or a future gate) can act on the
    # count rather than parse a sentence. No code in this repo reads `calendar_status` today
    # (checked: `scripts/production_gates.py`, `scripts/campaign_status.py`,
    # `scripts/production_reading.py`, and every test file were grepped for the key -- none of them
    # reference it), so this format change has no consumer to break.
    _measured_rows = sum(1 for row in rows if row["v100_completed_frames_per_second"] is not None)
    if _measured_rows == 0:
        _calendar_status = "BLOCKED_ON_MEASURED_V100_THROUGHPUT"
    elif _measured_rows == len(rows):
        _calendar_status = f"V100_THROUGHPUT_COMPLETE {_measured_rows}/{len(rows)}"
    else:
        _calendar_status = f"PARTIAL_V100_THROUGHPUT {_measured_rows}/{len(rows)}"
    return {
        "_comment": [
            ...
            "V100 throughput is null until measured on that host, from measured-v100-throughput.json;",
            "T4 rates are references, not estimates silently relabelled as production measurements.",
        ],
        "host_profile": "v100",
