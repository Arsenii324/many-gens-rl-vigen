...
def plan_row(baseline: str, frames: int, capacity: int | None, seeds: list[int]) -> dict:
    ...
    row["throughput_source"] = "measured" if fps else "unmeasured"
    if fps:
...
def main() -> int:
    ...
    print(json.dumps({
        "_comment": [
            ...
            "Costs cover only the ten baselines with measured throughput; alda and ctrl have no",
            "successful CUDA run yet and are deliberately absent from the total rather than",
            "estimated into it.",
        ],
        "frames": args.frames,
        "seeds": seeds,
