...
| `wait-and-train-v4.sh` | **runs on the host**, detached. Waits for a GENUINE vacancy — no foreign holder **and** ≥ `NEED` MiB free for `HOLD` consecutive samples of `gpu-occupancy.log`, the rule `capacity-check.sh` validated — then launches one cell through `train-production-cell-v5.sh` and arms `self-vram-cap.sh` on the container that appears. Refuses to launch when the occupancy log is stale (it would be blind to a co-tenant), when any `cell-c*` container of ours is up, when another launcher runs, when the tag's result archive exists, or when free disk is below `MIN_DISK_GIB`. `DECIDE_ONLY=1 AS_OF=<ts>` replays one past moment; `DRYRUN=1` walks the whole path and stops at the launch. All seven branches were exercised on 2026-09-17/18 before it was armed. It never stops or deletes anything |
| `train-production-cell-v6.sh` | v5 with two knobs and nothing else changed: `PAYLOAD` (v5 hardcoded `payload-v214-$FAMILY.tgz`, so a family whose payload was built later could not be launched without mislabelling it) and `PLACES365_DIR` (passed as `NATIVE_PLACES365_DIR_HOST`, mounted read-only at `/opt/places365`, which makes the cell skip the copy and extraction). Sets `NATIVE_PLACES365_SPLIT=train`, because production refuses `val` without an explicit deviation (A22). Its header carries the dry-run command; going through `launch-card-cell.sh` with `NATIVE_HOST_DRY_RUN` would arm watches for a cell that never starts |
| `capacity-check.sh` | **runs on the laptop**, read-only. Says whether each card is AVAILABLE for a training cell under the ten-minute vacancy rule, from the host's occupancy log: no foreign holder and at least `NEED` MiB free (default 11,421) for `HOLD` consecutive samples (default 10). `AS_OF=<YYYY-mm-ddTHH:MM>` replays a past moment. Validated on 2026-09-17 against two known moments: AVAILABLE at 11:00 (a real 15-minute absence), not available at 07:52 (the 4-minute gap in which a cell was stopped). It never launches anything |
| `watch-capacity.sh` | **runs on the laptop.** The same rule as `capacity-check.sh`, polled every 60 s while nothing of ours runs, so an opening is noticed without anyone asking. Reports `CAPACITY` (a card is available under the rule), `OPENING` (clear for 3–9 minutes; heads-up only), `NEW-GROUP` (a group not seen before), `LOGGER` (the occupancy log went stale, so the watcher is blind). `trip` exits on CAPACITY, NEW-GROUP or LOGGER; `beat` also prints a heartbeat every 15 polls. Ran continuously from the afternoon of 2026-09-17 in both modes. It never launches anything. Its list of known groups is hardcoded to the three seen on this host |
