...
| [C96](#c96) | the shared-evaluator ledger's one global code revision was both too broad and too narrow: a training-only patch invalidated every family, while a change to a family evaluator's local runtime could leave its revision unchanged; fixed with a family-specific static runtime closure/config binding and row-level canonical scope/measurement revisions, and the `v176` wave remotely validated all 7 families on the current closure (`production_gates.py`: PASS) | OURS + FALSE-CERTIFICATION | RESOLVED | `0ef59ab` |
...
**25 OPEN · 2 READY · 1 BLOCKED · 34 MONITORED · 36 RESOLVED · 98 total.**
