...
| [`EVAL-DECOMPOSITION.md`](EVAL-DECOMPOSITION.md) | **The reported number decomposed into every term it depends on, derived from its definition rather than accreted from noticing.** 28 terms across policy, environment, estimator, comparison and provenance, each with a status — and an UNEXAMINED column that is the point of the file. Says where the gaps concentrate: *which weights act, in what mode, with what step-dependence, in what env*, verified for the RL-ViGen five and unexamined for five families. | When asking "have we covered evaluation?", and before adding an axis to the seam audit — this is the list that is a consequence of the definition, so a gap in it is a gap in the derivation. |
...
| [`../notes/README.md`](../notes/README.md) | Current owner decision sheet, external-review triage, production gates, inference/checkpoint proposals, published Door anchor, and record-completeness specification. | During the final pre-production pass; use `scripts/production_gates.py` for live status. |
...
- The sibling `gen-rebuttal/vigen-idaac` project contains the related IDAAC and ALDA work; it is
  intentionally not linked here because it is not part of a clean checkout of this repository.
