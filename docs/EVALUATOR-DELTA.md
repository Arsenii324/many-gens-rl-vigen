...
| aspect | theirs | ours | delta |
|---|---|---|---|
| loop | `src/train.py::evaluate` | `run_scene_dmc_gb` | *unverified* — written from their env/action path, not read line-by-line against their loop |
| render | 100 | 100 (`--image-size` default) | none |
| N | `--eval_episodes` | our `--episodes` | **DELIBERATE**: we set N; theirs is a flag we also set |
