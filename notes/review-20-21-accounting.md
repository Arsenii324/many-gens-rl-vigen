...
| 7 | PPG sits between two identities; freeze it | **FIXED, and the review was more right than it knew** | Reading `raileanu21a-supp.pdf` §E directly: this port already ran §E's shared grid in every value that distinguishes the identities, each deviating from PPG's own release. Linear LR decay implemented and set to 1e6 env steps. The rollout geometry (8x256 vs §E's 1x2048) is the one remaining gap, deferred to the step-2 throughput measurement. A36 |
...
**Two evaluator attestations are now superseded by today's source changes — `alda` (the step-0
guard) and `ppg` (the decay) — and belong in the single final wave, per Q47.** No wave should run
before the geometry question in A36 is settled, since it would move `ppg` again.
