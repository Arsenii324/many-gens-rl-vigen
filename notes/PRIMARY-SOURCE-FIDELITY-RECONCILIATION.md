...
| Channels and stack | RL-ViGen and DMC-derived paths use RGB and usually a three-frame stack. The independent PPO/CTRL adapters receive one RGB observation whose channel dimension is 3; “3” in their paper is not silently inferred from this. `idaac/envs.py:124-133`, `alda` spec `:31-40`, `ppg` env path, `ctrl/train_ppo.py:209`. | **SOURCE VARIANT WITH CONTEXT / UNSUPPORTED ADAPTATION**. IDAAC’s DMC supplement explicitly uses 3 stacked frames; the current C1 IDAAC pilot must not be treated as evidence for the C2 frame-stack decision. |
...
| `idaac` / idaac | `families.json:244-373`; current constants 4 processes × 256 steps, 8 minibatches, log interval25; launcher 64×64 and eval-easy: `runnable/_launch/idaac.sh:9-46`; arguments and current Box support `runnable/idaac/ppo_daac_idaac/arguments.py:1-142`, `model.py:311-385`. | Descriptor `:282-373`: floor to 1024-frame rollout quantum, offline four regimes × ten scenes, endpoint-only native evaluation but retained 50k checkpoint copies. The source train loop saves only at terminal by default and logs/evaluates at `log_interval`: `ext/idaac/train.py:145-229`; the port adds retention. | Original Procgen recipe is 64 envs/worker, 256 rollout, gamma .999, lambda .95, 8 minibatches, entropy .01, lr5e-4, total25M, no frame stack (`paper_2102.10330`, p.15). The authors’ DMC supplement §E instead uses 3 stacked frames, one process, 2048 steps, 32 minibatches, PPO epochs10, entropy0, lr3e-4, and IDAAC coefficients αa=.1, αi=.1. Current Door is neither original Procgen nor that DMC recipe: it is a one-RGB-frame 64×64, 4-process/256-step port. This is the C1/C2 issue, not closed by a short pilot. |
...
| Optimization | Paper Table IV: batch RL128/SODA256, SAC lr1e-3, alpha1e-4, SODA3e-4, actor freq2, critic1, auxiliary2, tau.005, gamma.99. Active arguments `:17-55` contain those SODA-specific values but production uses common `train_steps=600k`, uncapped buffer, and project evaluation. | **SOURCE VARIANT WITH CONTEXT**. Check the effective argv and log for any descriptor override; source defaults alone are not sufficient evidence. |
| Test-time behavior | Paper p.9/algorithm says only the learned encoder/policy is used at test. Current offline evaluator loads the checkpoint without the training auxiliary update. | **EXACT SOURCE MATCH** for the separation of training auxiliary loss and evaluation behavior; common Door grid is project-owned. |
...
### 5.9 IDAAC — the unresolved one-stack versus DMC three-stack issue
...
| Procgen recipe | Paper p.15: gamma .999, λ .95, rollout256, 3 PPO epochs, 8 minibatches, entropy .01, lr5e-4, workers1/envs64, no frame stack, 25M frames. | Current descriptor sets 4 processes × 256 steps, 8 minibatches and log interval25: `families.json:247-308`; current defaults/arguments `runnable/idaac/ppo_daac_idaac/arguments.py:32-80`. | **SOURCE VARIANT WITH CONTEXT**: update geometry mostly follows the Procgen recipe, but process count and Door adapter differ. |
| DMC continuous recipe | The authors’ supplementary `ext/idaac/raileanu21a-supp.pdf`, §E: 3 stacked frames, one process, 2048 steps, 32 minibatches, PPO epochs10, entropy0, lr3e-4, linear decay, and IDAAC αa=.1/αi=.1. | Current Door port is one RGB frame in a 3-channel Box, 64×64 (`runnable/idaac/ppo_daac_idaac/envs.py:124-133`), 4×256/8 and current launcher does not apply the DMC §E 2048/32/10/0/3-stack recipe. | **SOURCE CONFLICT** if the current run is described as the authors’ continuous-control IDAAC recipe. **UNSUPPORTED/NECESSARY ADAPTATION** if explicitly described as a Door port. A C1 one-stack pilot does not resolve this; C2 is a separate 3-stack arm recorded in `notes/DECISION-SHEET.md:1486-1520`. |
...
**Required disposition:** keep the current one-stack run labelled IDAAC-P/C1 or equivalent, not
“faithful continuous-control IDAAC.” Before production ranking, either (a) build and review the
three-stack DMC-informed IDAAC-C2 configuration, or (b) ratify the one-stack Door port as the
explicit design point and report the DMC-source conflict. Do not let a short one-stack pilot settle
that choice.
...
| Procgen recipe | Paper p.14: gamma .999, λ .95, rollout256, minibatches8, entropy .01, clip .2, reward norm, lr5e-4, workers4/envs64, total100M, no frame stack. | Current descriptor passes 8 envs × 256, `interacts_total=600k`: `families.json:484-534`; current source defaults retain algorithmic values but launcher overrides env count: `ppg.sh:15-42`. | **SOURCE VARIANT WITH CONTEXT**: 8 envs is a resource/domain adaptation and 600k is the common Door budget, not the 100M Procgen experiment. |
...
| Observation geometry | Procgen source is 64×64 RGB and no frame stack; current launcher requests 64 and the Door adapter supplies one RGB frame: `ppg.sh:15-28`, `runnable/ppg/.../envs.py`. | **SOURCE VARIANT WITH CONTEXT**: resolution matches the source family, domain and stack semantics do not. |
...
* IDAAC’s main Procgen recipe is not the authors’ continuous-control recipe. The supplement’s DMC
  ...
  2048 steps, 32 minibatches, 10 PPO epochs, entropy0 and lr3e-4, plus IDAAC coefficients. Current
  Door IDAAC is 4×256/8 and one RGB frame. That is a genuine unresolved design point, not an
  implementation detail.

The current one-stack IDAAC pilot can establish importability, checkpointing, and that the port
runs. It cannot license the claim “we reproduced continuous-control IDAAC.” A C2 three-stack arm,
or explicit owner ratification of the one-stack Door design point with its source conflict, is
...
| idaac | IDAAC algorithm and Procgen-shaped PPO defaults | Box head and Door adapter | **C1 one-stack/4×256 is not DMC §E; C2 frame-stack/update choice unresolved** | “IDAAC Door port; continuous-control fidelity design point pending/declared” |
| ppg | PPG phasic schedule and PPO coefficients | Box head, 8 envs, 600k, 64 Door | .01 entropy is discrete-source value; auxiliary phase count at 600k | “PPG phasic source algorithm with continuous Door adaptation” |
