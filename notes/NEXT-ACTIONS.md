...
| 5 | Attach run provenance to offline rows; fix evaluator identity | **done.** Offline rows are enriched from `run_manifest.json` during bundling (loud, non-fatal if absent — tested both paths); `families.json` joined `REVISION_MEMBERS` because omitting per-family eval configuration made the revision *under*-sensitive |
