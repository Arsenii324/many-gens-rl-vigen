...
| H6 | First native cell: `drqv2` s101 on card 0 (booked for our group), by hand via the v6 wrapper with an explicit `TIMEOUT_S` and disk allowance, dry run first | **RUNNING since 09:46 MSK 20 Sep** (`card0-20260920-094616`), dry run passed, recorded, watched (floor 77). Owed from it: first V100 throughput of a native (FPS from `train.log`) written into `production-schedule-v100.json`'s terms; measured RAM at full replay; then collection as for F5 | launched, recorded, watched |
