...
| D4 | O1, the Places365 train corpus on the host | **IN PROGRESS 2026-09-18 06:25** | The blocker was the SOURCE, not the host: measured from a container, GitHub gives 13.7 MB/s and data.csail.mit.edu 623 B/s. The laptop's validated copy (26 GB, 1.8M files, 365 classes) is streaming to `~/rlvigen-assets/places365-train` via tar into an extracting container, 2.4 MB/s, ETA ~09:30. Closes when `verify_datasets --split train` prints `class directories: 365` and PASS **on the host**, after a chmod -R a+rX (the extractor writes as root). |
