...
Rung 1 then found two more, both in the fixes themselves: `_dev_of` validated `stat -f -c %i`
against `^[0-9]+$` while GNU stat prints that ID in **hex** (`c4aaf1bac0c3eb66`), so the mirror
device check reported "unreadable" on every real run and could never reach the same-device branch;
and `--gpus none`, an option this script's own refusal advertises, is invalid docker.

**Five defects, none of them findable from this laptop.** That is the argument for the ladder, and
it is why the remaining rungs get run rather than reasoned about.
...
2. **No durable second location exists.** One filesystem with >=20 GB free, so
   `NATIVE_ACCEPT_SAME_DEVICE=1` is permanent here. That is an owner-level property of the
   campaign, not an operator setting.
3. **Three seeds is unproven.** `notes/seed-variance-at-100k.md`: eval-easy 95% CI includes zero at
   n=3. Widening costs 12 cells per seed.

None of the three is code. All three are decisions or other people's processes.
