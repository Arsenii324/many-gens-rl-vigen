...
|---|---|---|---|---|
| `rlvigen` | drqv2, svea, sgqn, drq, curl, pieg | agent (networks + optimizers), timer, _global_step, _global_episode | on disk during the run, never in the checkpoint and never retained | **NOT a full restart -- empty buffer** |
...
**Only `rlvigen` sets `preserve_snapshots` (100000).** `family.py` emits
`RLVIGEN_PRESERVE_SNAPSHOTS` only when the value is non-null (its line ~372), so the other six
families retain every stamp. The curve is therefore **seven points for the RL-ViGen five and
thirteen for everyone else** — see the second generated table in §6.

That asymmetry is a cross-family seam, not just a storage note: a joint curve figure over all
twelve baselines has two different x-grids, and the common one is 100k. Recorded in
`PRODUCTION-HOST-RATIFICATION.md` as an open item — either give the other six a matching
`preserve_snapshots`, or plot the common 100k grid and say so. The `rlvigen` value itself is
deliberate (`families.json`: "keeping all twelve would double that for a curve twice as dense as
any plot needs").
