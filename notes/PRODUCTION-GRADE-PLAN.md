...
| `runtime_import_manifest` → digest per row, manifest emitted once | 74 % of a 19.7 MB bundle; 2 distinct values over 569 rows; nothing reads it | `scripts/eval_grid.py` (CODE) | **deliberately deferred 2026-09-10.** It is storage, not measurement, and the fixed bundle is already ~19 KB/row. It should ride the NEXT member change — the frame-0 checkpoint work requires one anyway — rather than widen a bump whose other three items are correctness fixes |
