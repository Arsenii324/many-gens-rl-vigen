...
| Q6 | **Do the `medium`/`low` faithfulness rows hold?** | The reconciliation covered `high` and the two marked FIXED. `rad`, `soda`, `alda`, `drq`, `ibac_sni` detail rows are unchecked | [`faithfulness-reconciliation.md`](faithfulness-reconciliation.md) |
