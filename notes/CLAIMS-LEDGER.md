...
| `sgqn` | SGQN (RL-ViGen) | `aux_lr` 1e-4 vs canonical 3e-4 — the catastrophic 0.3 is fixed, a **3× divergence on the shared encoder remains**; replay cap; Places split | Executed values are RL-ViGen's own: `aux_lr` **1e-4**, `sgqn_quantile` **0.93**, `aux_beta` **0.99** — FAITHFULNESS's "FIXED to 8.0e-5 / 0.9" describes the legacy `rlgen/` path, which production does not run
...
| `idaac` | **IDAAC adapted to a target without level identity** | Rollout 16× smaller; `level_seed` is now episode-scoped — internally coherent but **an episode is not a Procgen level**, so the invariance construct is target-authored |
...
| `ctrl` | CTRL (repaired) | Two lines restored that were commented out upstream — **without them `ctrl_public @ 7a118c8` cannot run its own algorithm**; 4× smaller rollout; MYOW draws from the *nearest* neighbouring cluster repeatedly (upstream's own quirk) | Online eval advances the JAX PRNG that the next `update_ppo`/`update_cluster` consumes, so the training trajectory is a function of the eval code path; **this is upstream's own structure** (`ctrl_public/train_ppo.py:193,202`) and is deliberately unpatched, so the isolation guard covers the placement stream only **Its evaluated return was in normalized units until 2026-09-05** (the common evaluator inherited `normalize_rewards=True` and summed the outermost VecNormalize reward); any ctrl evaluation number predating that fix is void, not merely incomparable
