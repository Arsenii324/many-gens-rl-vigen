...
| `idaac` | **IDAAC (DMC continuous-control profile)** [Updated 2026-09-06, A35/A65/Q55: production now runs the authors' own published DMC continuous-control recipe (`num_processes=1`, `num_steps=2048`, `ppo_epoch=10`, `frame_stack=3`, linear LR decay, etc.), not the reduced Procgen-parser adaptation this row previously described — **not** a smaller rollout any more, `num_processes=1` now matches the paper exactly. **Not yet validated by a full-length training run**, see DECISION-SHEET A35's PILOT RESULT/IMPLEMENTED notes.] [Added 2026-09-07, prompted by the owner asking whether the scheduler comes from the paper or the code — it is the paper: **`ext/idaac/train.py` constructs all three optimizers with a constant `lr=args.lr` and contains no scheduler; `update_linear_schedule` exists only in our clone**. The linear decay is therefore OUR addition, taken from the supplement, and is a deviation from the released code toward the publication — the opposite direction from this project's usual rule. The governing principle, stated once here and applied to `ppg` the same way: **each method follows its own authors' most relevant published guidance, and where its own authors are silent on this target, its released code stands.** IDAAC's authors published a DMC continuous-control recipe and this is that target, so their recipe governs; OpenAI's PPG authors never addressed continuous control, so PPG's released constant LR stands (A36). Without that principle the two are simply inconsistent.] | `level_seed` is episode-scoped — internally coherent but **an episode is not a Procgen level**, so the invariance construct is target-authored (unchanged by the C2 transition, a separate adaptation) |
