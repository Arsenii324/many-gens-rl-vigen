...
**Four baselines have production-length rows. Four cells count as DONE, one as PARTIAL, one is trained with its evaluation deferred.**
...
| `idaac` | 103 | complete (DONE) | 598: 484 curve + 88 endpoint + 26 training-curve rows | trained 4 h 45 min, finished 17:45 on 2026-09-20; evaluated beside a co-tenant's 21.5 GiB without incident |
...
[`production-host/39`](production-host/39-idaac-at-three-seeds.md) — no seed reaches competence).
`drqv2` s101, the first RL-ViGen native, is TRAINED to 600k (31 FPS, 5.37 h, 38.5 GiB RAM; it
solves the training scene by ~150k frames) and its EVALUATION IS DEFERRED: 12 checkpoints plus
`snapshot.pt` are retained on the host and on the laptop (`fetched/card0-20260920-094616`); run it
from them with `host-scripts/reeval-cell*.sh` when a card is ours. `svea` s101 failed twice on
19 Sep (a `run_probe.sh` bug, fixed; then the host crashed) and must not be relaunched as it was.
...
ample for the training scene; for the four on-policy baselines it is 0.4–7.5 % of their papers'
budgets and every completed one fails the competence gate. Evaluation costs twice the training of a
fast baseline because it runs ~2,400–3,200 episodes serially on one core with the GPU idle — not
because the grid is arbitrary; parallelising by regime is the chosen remedy (`DECISION-SHEET.md`
A68). The owner's decisions and the working values set for them are one list:
`python scripts/open_decisions.py` (A59–A72), linked from §7b.

**The older narrative below is kept because it is still true of the cells it describes.** The
on-policy queue it discusses (`idaac` s103 — now done; `ibac_sni` s102 from zero and s103; `ppg`'s
two seeds) comes AFTER the natives, by the owner's ruling of 20 Sep (A60).
