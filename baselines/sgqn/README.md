...
| status | `implemented` |
| backbone | `drqv2` |
| paper | Bertoin et al. 2022, arXiv:2209.09203 |

A real, distinct training rule. Trainable.

Its saliency machinery imports captum at module scope; nothing upstream declares that.

## Run

```bash
bash baselines/sgqn/train.sh Door 0
```
...
**This baseline needs 1 package(s) beyond the shared set:**

```bash
pip install captum
```

Discovered by running it, not by reading: nothing upstream declares them, and the import happens at module scope so the failure is at construction time.
...
`configs/vigen.yaml:sgqn` — `base` plus only the keys below.
