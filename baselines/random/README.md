...
Negative control. Run it first, on every task, and gate every ratio on it.

## Run

```bash
bash baselines/random/train.sh Door 0
```
...
`configs/vigen.yaml:random` — `base` plus only the keys below.
